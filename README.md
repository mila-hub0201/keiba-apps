...
インストール済みなら「ウマフル」はAndroidの**共有先アプリ**として登録される。
JRAサイトから保存した出馬表PDFを「ファイル」アプリ等で開き、
**共有 → ウマフル** を選ぶだけで解析が始まる(ファイル選択の操作が不要)。
リンク(URL)の共有ではPDF本体を受け取れないため、PDFファイル自体を共有すること。
